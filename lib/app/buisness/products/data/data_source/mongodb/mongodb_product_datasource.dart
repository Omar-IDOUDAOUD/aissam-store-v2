import 'dart:async';

import 'package:aissam_store_v2/app/buisness/products/core/constants.dart'; 
import 'package:aissam_store_v2/app/buisness/products/core/failures.dart';
import 'package:aissam_store_v2/app/buisness/products/data/data_source/product_datasource.dart';
import 'package:aissam_store_v2/app/buisness/products/data/models/category.dart';
import 'package:aissam_store_v2/app/buisness/products/data/models/product_details.dart';
import 'package:aissam_store_v2/app/buisness/products/data/models/product_preview.dart';
import 'package:aissam_store_v2/app/buisness/products/core/params.dart';
import 'package:aissam_store_v2/app/core/data_pagination.dart';
import 'package:aissam_store_v2/config/databases/mongo_db.dart';
import 'package:mongo_dart/mongo_dart.dart';

class ProductsRemoteDatasourceImpl implements ProductsDatasource {
  final MongoDb _mongodb;

  ProductsRemoteDatasourceImpl(this._mongodb);

  Future<DbCollection> _getCollection(String collactionName) async {
    final db = await _mongodb.db;
    return db.collection(collactionName);
  }

  Future<DbCollection> get _productsCollection => _getCollection('products');
  Future<DbCollection> get _categoriesCollection =>
      _getCollection('categories');

  SelectorBuilder _buildQuery(DataPaginationParams paginationParams,
      {List<String>? fields}) {
    var builder = where
        .limit(paginationParams.pageSize)
        .skip(paginationParams.indexIdentifierObj);
    if (fields != null) builder = builder.fields(fields);
    return builder;
  }

  DataPagination<T> _buildPagination<T>(
      List<Map<String, dynamic>> data,
      T Function(Map<String, dynamic>) fromMap,
      DataPaginationParams paginationParams) {
    final convertedData = data.map((e) {
      return fromMap(e);
    }).toList();
    final pagination = DataPagination<T>(
      items: convertedData,
      hasNextPage: data.length == paginationParams.pageSize,
      indexIdentifier: paginationParams.indexIdentifierObj + data.length,
    );
    return pagination;
  }

  @override
  Future<DataPagination<CategoryModel>> categories(
      GetCategoriesParams params) async {
    final coll = await _categoriesCollection;
    final res = await coll.find(_buildQuery(params.paginationParams)).toList();

    return _buildPagination<CategoryModel>(
        res, CategoryModel.fromJson, params.paginationParams);
  }

  @override
  Future<DataPagination<ProductPreviewModel>> productsByCategory(
      GetProductsByCategoryParams params) async {
    final coll = await _productsCollection;
    final res = await coll
        .find(_buildQuery(params.paginationParams,
                fields: ProductPreviewModel.fields)
            .eq('categories', params.category))
        .toList();

    return _buildPagination<ProductPreviewModel>(
        res, ProductPreviewModel.fromJson, params.paginationParams);
  }

  @override
  Future<DataPagination<ProductPreviewModel>> productsByPerformance(
      GetProductByPerformanceParams params) async {
    final coll = await _productsCollection;
    var query = _buildQuery(params.paginationParams,
        fields: ProductPreviewModel.fields);

    if (params.performance == ProductsPerformance.bestSellers)
      query = query.sortBy('sales', descending: true);
    else if (params.performance == ProductsPerformance.newArrivals)
      query = query.sortBy('created_at', descending: true);
    else if (params.performance == ProductsPerformance.trending)
      query = query.sortBy('views', descending: true);
    else
      query = query
          .sortBy('review_count', descending: true)
          .sortBy('average_rating', descending: true);
    final res = await coll.find(query).toList();

    return _buildPagination<ProductPreviewModel>(
        res, ProductPreviewModel.fromJson, params.paginationParams);
  }

  @override
  Future<DataPagination<ProductPreviewModel>> searchProducts(
      SearchProductsParams params) async {
    final coll = await _productsCollection;
    var query = _buildQuery(params.paginationParams,
        fields: ProductPreviewModel.fields);
    final keywords = params.keywords.split(' ');

    // var nameQuery = query;
    // var descriptionQuery = null;
    bool firstRound = true;
    for (final keyword in keywords) {
      final nameMatcher = where.match('name', '.*$keyword.*');
      query = firstRound ? nameMatcher : query.or(nameMatcher);
      final descriptionMatcher = where.match('description', '.*$keyword.*');
      query = query.or(descriptionMatcher);
      firstRound = false;
    }

    if (params.categories != null && params.categories!.isNotEmpty)
      query = query.and(where.oneFrom('categories', params.categories!));
    if (params.sizes != null && params.sizes!.isNotEmpty)
      query = query.and(where.oneFrom('sizes', params.sizes!));
    if (params.colorName != null && params.colorName!.isNotEmpty)
      query = query.and(where.oneFrom('available_colors', params.colorName!));
    query = query.and(where.inRange(
        'price', params.minPrice ?? 0, params.maxPrice ?? double.infinity));

    final res = await coll.find(query).toList();

    return _buildPagination<ProductPreviewModel>(
        res, ProductPreviewModel.fromJson, params.paginationParams);
  }

  @override
  Future<ProductDetailsModel> product(String id) async {
    final coll = await _productsCollection;
    final Map<String, dynamic>? res =
        await coll.findOne(where.id(ObjectId.fromHexString(id)));
    if (res == null) throw ProductNotFoundFailure();
    return ProductDetailsModel.fromJson(res);
  }
}
